...
Read `<config-root>/plugins/client-status.user-context.md`. If missing, route to `/setup-status` and stop. Read shared identity at `<config-root>/memory/me/identity.md` and shared voice at `<config-root>/memory/me/voice.md`.
